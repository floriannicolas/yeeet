...
router.post('/upload', requireAuth, upload.single('chunk'), async (req: Request, res: Response): Promise<void> => {
    if (!req.file) {
...
router.post('/files/:id/toggle-expiration', requireAuth, async (req: Request, res: Response): Promise<void> => {
    try {
        let userId = -1;
        const token = getTokenFromRequest(req);
        if (token) {
