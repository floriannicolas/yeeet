...
export default function CookieConsent({ onAcceptCallback = () => { }, onDeclineCallback = () => { } }) {
    ...
    const [areCookiesConsent, setAreCookiesConsent] = useLocalStorage<undefined|boolean>(COOKIE_CONSENT_KEY, undefined);
    ...
    useEffect(() => {
        try {
            setIsOpen(true);
            if (areCookiesConsent !== undefined || document.cookie.includes(`${COOKIE_CONSENT_KEY}=true`)) {
                setIsOpen(false);
                setTimeout(() => {
                    setHide(true);
                }, 700);
