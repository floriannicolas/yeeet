...
const filesReducer = (files: FileInfo[], action: FileReducerAction): FileInfo[] => {
  ...
  switch (type) {
    ...
    case 'UPDATE':
      return files.map((file) => (file.id === item.id ? item : file));
...
export default function Dashboard({ session }: { session: Session }) {
      ...
      try {
        ...
        if (data.status === 'completed') {
          ...
          toast({
            ...
            description: (
              <>
                Your file <span className="font-semibold">{data.originalName}</span> is now
                available.
              </>
            ),
          });
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
    >
      <Header storageInfo={storageInfo} progress={progress} lastAppVersion={lastAppVersion} />
        ...
        <div className="flex items-center justify-between space-y-2 md:space-y-0 mb-6">
          <div>
            ...
            <p className="text-md font-light text-muted-foreground">
              Access & manage your screenshots & files!
            </p>
          </div>
              ...
              <div className="flex w-full items-center justify-center relative gap-2">
                ...
                >
                  ...
                  <Input
                    ...
                    multiple
                    onChange={(e) =>
                      e.target.files?.[0] && uploadQueue.manage(Array.from(e.target.files))
                    }
                  />
                </Label>
                <TooltipProvider>
