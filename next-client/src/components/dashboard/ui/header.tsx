...
import { FlameKindling, LogOut, AppWindowMac, EllipsisVertical, X } from 'lucide-react';
...
export default function Header({
  ...
  const [isAppNewVersionAlertClosed, setIsAppNewVersionAlertClosed] = useSessionStorage(
    IS_APP_NEW_VERSION_ALERT_CLOSED,
    false
  );
      ...
      {showAppUpdate && (
        <Alert className="rounded-none">
          ...
          <AlertDescription className="text-sm text-muted-foreground">
            A new desktop app is available for macOS! you can download it{' '}
            <span
      ...
      <header className="flex sticky top-0 bg-background z-10 h-16 shrink-0 items-center gap-2 border-b px-6">
        ...
        {progress > 0 && (
          <div className="absolute left-0 right-0 top-full border-b border-t bg-background overflow-hidden">
            <div
              className="left-0 right-0 top-full h-2 bg-gray-200 text-right transition-all"
